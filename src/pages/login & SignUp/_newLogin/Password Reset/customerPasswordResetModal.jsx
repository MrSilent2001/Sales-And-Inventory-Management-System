...
const CustomerResetModal = forwardRef((props, ref) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                const customer = await axios.get(`http://localhost:9000/customer/findCustomerByEmail/${formData.email}`);

                const id = customer.data.id;
                const randomFourDigitNumber = Math.floor(1000 + Math.random() * 9000);
                const password = `Trade${randomFourDigitNumber}`;

                await axios.put(`http://localhost:9000/customer/reset/${id}`, {
                    password: password
                }, {
                    headers: {
...
export default CustomerResetModal;
